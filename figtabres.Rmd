...
- Number of intermittent gauges taking in account only post-1800 (average number of years of data): 
...
```{r envdistrib, fig.width = 10, fig.height=9, dpi=600, message=FALSE, echo=FALSE}
loadd(envhist)
grid.newpage() 
grid.draw(envhist)
```
...
```{r variable-importance, fig.width=12, fig.height=10, echo=FALSE}
loadd(vimp_plot_u10)
loadd(vimp_plot_o1)

p_u10 <- vimp_plot_u10 + 
  ggtitle(bquote('A. Gauges with mean annual discharge < 10'~m^3~s^-1)) +
  theme(axis.title.x = element_blank(),
        plot.margin = unit(c(-0.25,0,0,0), unit='in'))
p_o1 <- vimp_plot_o1 + 
    theme(legend.position = 'none') + 
  ggtitle(expression(B.~Gauges~with~mean~annual~discharge >= 1~m^3~s^-1)) + 
  theme(plot.margin = unit(c(0,0,0,0), unit='in'))
...
```{r prediction-uncertainty-by-variable, fig.width=9, fig.height=9, echo=FALSE}
loadd(gaugeIPR_plot)
grid.newpage()
...
```{r comparison, echo = FALSE, fig.width = 5, fig.height = 3  dpi=300}
loadd(fr_plot)
loadd(us_plot)

p_bind <- fr_plot/us_plot
p_out <- p_bind + 
```

#Comparison of results with on-the-ground observations for PROSPER
```{r QC, echo = FALSE, fig.width = 8, fig.height=6, dpi=300}
loadd(pnw_plot)
print(pnw_plot)
```

#Comparison of results with on-the-ground observations for ONDE
```{r QC, echo = FALSE, fig.width = 8, fig.height=8, dpi=300}
loadd(onde_plot)
print(onde_plot)
```
