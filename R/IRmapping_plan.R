...
plan <- drake_plan(
  ...
  tasks_classif = target(
    create_taskclassif(in_gaugestats = in_gauges,
  ...
  tasks_regr = target(
    create_taskregr(in_task_classif,
                    oversample=FALSE),
    transform = cross(
      in_task_classif = c(task_classifo10, task_classifu10),
  ...
  baselearners_classif = target(
    create_classiflearners(task_classif),
    transform = map(
      task_classif = c(task_classifu10, task_classifo10),
      .names = c('learners_classifu10', 'learners_classifo10'))
  ),

  baselearners_regr = target(
    create_regrlearners()
  ),
  ...
  #seplearners_classif = c(learners_classifu10, learners_classifo10), #Subdivide the baselearners dynamic targets

  autotuningset = target(
    set_tuning(in_learner = seplearners,
               in_measures = measures,
               nfeatures = length(tasks$feature_names),
               insamp_nfolds = 4, insamp_neval = 10,
               insamp_nbatch = parallel::detectCores(logical=FALSE)-1
    ),
    transform = map(seplearners = c(learners_classifu10, learners_classifo10),
                    tasks = c(task_classifu10, task_classifo10),
                    .names = c('autotuning_u10', 'autotuning_o10')
    )
  )
  ,
  ...
  resamplingset = target(
    set_cvresampling(rsmp_id = 'repeated_cv',
                     ...
                     outsamp_nrep = 1,
                     outsamp_nfolds = 2),
    transform = map(task = c(task_classifu10, task_classifo10),
                    .names = c('resampling_u10', 'resampling_o10')
    )
  )
  ,
  ...
  rfresampled_classif = target(
    dynamic_resample(in_task = in_task,
                     in_learner = in_autotuningset,
                     in_resampling = in_resampling,
                     store_models = TRUE,
                     type = 'classif'),
    transform = map(in_task = c(task_classifu10, task_classifo10),
                    in_autotuningset = c(autotuning_u10, autotuning_o10),
                    in_resampling = c(resampling_u10, resampling_o10),
                    .names = c('rfclassif_u10', 'rfclassif_o10')
    )
  ),
  # ,
  #
  ...
  #
  rfbm_classif = target(
    combine_bm(in_resampleresults = list(in_rf),
               out_qs = outpath_rfclassif,
               id_suffix),
    transform = map(in_rf = c(rfclassif_u10, rfclassif_o10),
                    id_suffix = c('_u10', '_o10'),
                    .names = c('rfbmclassif_u10',
                               'rfbmclassif_o10')
    )
  ),
  #
  ...
  resamplingset_featsel = target(
    ...
    transform = cross(in_strategy = c('repeated_cv', "repeated-spcv-coords"),
                      task = c(task_classifu10, task_classifo10),
                      in_outrep = 1,
                      in_outfolds = 3,
                      .names = c('featsel_cvu10', 'featsel_spcvu10',
                                 'featsel_cvo10', 'featsel_spcvo10'))
  )
  ,
  ...
  tasks_featsel = target(
    ...
    transform = map(rfbm = c(rfbmclassif_u10, rfbmclassif_o10),
                    task = c(task_classifu10, task_classifo10),
                    .names = c('task_featselu10', 'task_featselo10')
  ...
  rfresampled_featsel = target(
    dynamic_resamplebm(in_task = in_taskfeatsel,
                       ...
                       type = 'classif'),
    transform= map(
      in_taskfeatsel = c(task_featselu10, task_featselu10,
                         task_featselo10, task_featselo10),
      rfbm_classif = c(rfbmclassif_u10, rfbmclassif_u10,
                       rfbmclassif_o10, rfbmclassif_o10),
      in_resampling = c(featsel_cvu10, featsel_spcvu10,
                        featsel_cvo10, featsel_spcvo10),
      .names = c('res_featsel_cv_u10', 'res_featsel_spcv_u10',  #'res_all_cvu10',
                 'res_featsel_cv_o10', 'res_featsel_spcv_o10')) #'res_all_spcv',
  )
  ,
  #
  ...
  rfeval_featsel = target(
    c(res_featsel_cv, res_featsel_spcv), #Cannot use combine as lead to BenchmarkResult directly in the branching
  transform = map(
    res_featsel_cv = c(res_featsel_cv_u10, res_featsel_cv_o10),
    res_featsel_spcv = c(res_featsel_spcv_u10, res_featsel_spcv_o10),
    .names = c('rfeval_featsel_u10', 'rfeval_featsel_o10')
    )
  ),
  #
  ...
  rfbm_featsel = target(
    analyze_benchmark(in_bm = rfeval_featsel,
                      in_measure = measures$classif),
    transform = map(rfeval_featsel,
                    .names = c('rfbm_featsel_u10', 'rfbm_featsel_o10'))
  ),

  interthresh_u10 = target(rfbm_featsel_u10$interthresh_dt[
    learner == selected_learner,
    max(thresh)]),

  interthresh_o10 = target(rfbm_featsel_o10$interthresh_dt[
    learner == selected_learner,
    max(thresh)]),
  ...
  rftuned = target(
    selecttrain_rf(in_rf = res_featsel_cv,
                   ...
                   insamp_nfolds = 4,
                   insamp_nevals = 10),
    transform = map(res_featsel_cv = c(res_featsel_cv_u10, res_featsel_cv_o10),
                    .names = c('rftuned_u10', 'rftuned_o10')
    )
  )
  ,
  ...
  #
  vimp_plot = target(
    ggvimp(in_rftuned = rftuned,
           in_predvars = predvars,
           varnum=25, spatial_rsp = FALSE),
    transform = map(
      rftuned = c(rftuned_u10, rftuned_o10),
      .names = c('vimp_plot_u10', 'vimp_plot_o10')
    )
  )
  ,
    #
  ...
  #  ),
  #
  bin_rftunedmisclass = target(
    bin_misclass(in_rftuned = rftuned,
                 in_gaugestats = in_gauges,
                 binvar = 'dis_m3_pyr',
                 binfunc = 'manual',
                 binarg = binarg,
                 interthresh=interthresh,
