...
scales_x <- list(
  ...
  dis_m3_pmn = scale_x_sqrt(breaks=c(0, 10^(0:log10(max(gaugepred$dis_m3_pmn)))),
                            labels=c(0, 10^(0:log10(max(gaugepred$dis_m3_pmn))))),
  dis_m3_pyr = scale_x_sqrt(breaks=c(0, 10^(0:log10(max(gaugepred$dis_m3_pmn)))),
                            labels=c(0, 10^(0:log10(max(gaugepred$dis_m3_pmn))))),
...
coordcart <- lapply(varstoplot_hist, function(var) {
  coord_cartesian(xlim=as.data.table(gaugepred)[, c(min(get(var)), max(get(var)))])
  }) %>%
  setNames(names(scales_x))
coordcart[['clz_cl_cmj']] <-  coord_cartesian(
  xlim=c(1,max(gaugepred$clz_cl_cmj)))
...
ggenvhist <- function(vartoplot, in_gaugedt, in_rivdt, in_predvars, intermittent=TRUE) {
  ...
  format <- as.data.table(in_dt)[, vartoplot2, with=F] 
  
  
  
  
  penvhist <- ggplot(dtformat, aes_string(x=vartoplot)) +
    geom_histogram(bins=20) + #aes(fill=intermittent), position = 'stack') - doesn't work with log or sqrt y scale
...
penvhist_grobs <- lapply(varstoplot_hist, ggenvhist, in_dt=gaugepred, in_predvars=predvars)
