...
#Compute bbrier and time adj
bm_timeadj <- function(bmres) {
  lrns_dt <- as.data.table(bmres$learners)
  
  nevals_inner <- lapply(lrns_dt$learner, function(lrn) {
    
    
    if ('instance_args' %in% names(lrn)) {
      nevals <- lrn$instance_args$resampling$iters*
        lrn$instance_args$terminator$param_set$get_values()$n_evals
    } else {
      nevals <- 1
    }
    return(nevals)
  ...
  lrns_time <- lrns_dt[, nevals_in := nevals_inner] %>%
    .[, list(nevals_in=mean(nevals_in)), by=learner_id]
    
  bm_ttot <- as.data.table(bmres$aggregate(list(msr('time_train'),
                                               msr('time_predict')))) %>%
    .[lrns_time, on='learner_id'] %>%
    .[, time_train := time_train/nevals_in]
...
bm_time_aucbbrier <- function(bmres) {
...
tbbrier_classif1 <- bm_time_aucbbrier(rfbm_classif$bm_classif)
tbbrier_regr1 <- bm_time_aucbbrier(rfbm_regr$bm_regr)
tbbrier_classif2 <- bm_time_aucbbrier(rfeval_featsel$bm_classif)
