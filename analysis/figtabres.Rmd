...
gaugebufras <- file.path(resdir, 'GRDCstations_bufras.tif')
...
#---- Import HydroSHEDS landmask raster ----------------------------------------
hydromask <- raster(hydrosheds_mask)
...
```{r models benchmarking curves, fig.width=12, fig.heecho=FALSE}
...
dat.fit  <- fit.variogram(var_smpl, fit.ranges = TRUE, fit.sills = FALSE,
                          vgm(varmods[!(varmods %in% c('Pow', 'Int'))]), 
                          fit.kappa = TRUE)
# The following plot allows us to assess the fit
plot(var_smpl, dat.fit)

# Convert hydromask to SpatialGrid
grd_proj <- projectRaster(hydromask, crs=crs_aeqd, res=500) %>%
  as('SpatialGrid')
