...
bm_timeadj <- function(bmres) {
  ...
  nevals_inner <- lapply(lrns_dt$learner, function(lrn) {
    ...
    #If no inner sampling
      ...
      inner_smp <- data.table(nruns = 1) 
      
      as.data.table(lrn$param_set)[, tstrsplit(id, '.', fixed=TRUE)]
      lrn$param_set
  ...
  lrns_time <- cbind(lrns_dt, nevals_inner) %>%
...
rsmp_bacc <- function(bmres, rsmp_i, threshold) {
  rsmp <- bmres$resample_result(rsmp_i)
  rsmp_pred <- rsmp$prediction()
  ...
  response <- as.data.table(prob_resp)[
    , c(fifelse(prob_resp>=threshold, '1', '0'))] 
  
  bacc <- mlr3measures::bacc(factor(as.character(rsmp_pred$truth), 
                                    levels=c('0', '1')),
                             factor(response, levels=c('0','1')))
  return(data.table(bacc=bacc, threshold=threshold))
}

bm_msrtab <- function(bmres, threshold) {
  ...
  bacc_vec <- lapply(1:bmres$n_resample_results, function(i) {
    baccthresh <- ldply(seq(0,1,0.01), function(threshold) {
      rsmp_bacc(bmres, rsmp_i=i, threshold=threshold)}) %>%
      setorder(bacc) %>%
      setDT %>%
      .[, .SD[.N, .(bacc, threshold)]]
  }) %>%
